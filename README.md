...
order of simplicity to complexity, with az_cicd later retrofitted with security
scanning to close specific gaps against a target role.

## Tasks

1. **docker_webserver** — containerised web server, Docker fundamentals
2. **az_cicd** — automated CI/CD pipeline on Azure (Pipelines, Container Registry, App Service), later retrofitted with Trivy, Dependabot, and Snyk security scanning
